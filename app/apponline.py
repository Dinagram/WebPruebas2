...
app.config.from_object(ConfigOnline)
app.permanent_session_lifetime = timedelta(seconds=app.config.get("CLIENT_ACCESS_TOKEN_MAX_AGE", 3600))
...
AUTH_SOURCE_KEY = "auth_source"
CLIENT_ACCESS_USER_KEY = "client_access_user"
CLIENT_ACCESS_GRANTED_AT_KEY = "client_access_granted_at"
...
def _clear_client_access_session():
    session.pop(CLIENT_ACCESS_USER_KEY, None)
    session.pop(CLIENT_ACCESS_GRANTED_AT_KEY, None)


def _mark_login_session():
    session[AUTH_SOURCE_KEY] = "login"
    _clear_client_access_session()


def _mark_client_access_session(user):
    session.permanent = True
    session[AUTH_SOURCE_KEY] = "token"
    session[CLIENT_ACCESS_USER_KEY] = user.username
    session[CLIENT_ACCESS_GRANTED_AT_KEY] = int(datetime.utcnow().timestamp())


def _has_valid_client_session(user):
    if not current_user.is_authenticated or current_user.username != user.username:
        return False

    session_user = session.get(CLIENT_ACCESS_USER_KEY)
    granted_at = session.get(CLIENT_ACCESS_GRANTED_AT_KEY)
    max_age = int(app.config.get("CLIENT_ACCESS_TOKEN_MAX_AGE", 3600))

    if session_user != user.username or granted_at is None:
        return False

    if int(datetime.utcnow().timestamp()) - int(granted_at) > max_age:
        logout_user()
        _clear_client_access_session()
        return False
...
def validate_client_access(user, token):
    if (
        ...
        and session.get(AUTH_SOURCE_KEY) == "login"
    ):
        return True, None

    if _has_valid_client_session(user):
        return True, None
    ...
    login_user(user)
    _mark_client_access_session(user)
    return True, None
...
def load_user(id):
    return ModelUser2.get_by_id(int(id))
...
def login():
    if request.method == 'POST':
        ...
        logged_user = ModelUser2.login(username, password)
        if logged_user != None:
...
def completar_trabajo(nombre):
    ...
    access_ok, access_error = validate_client_access(user, token)
    if not access_ok:
        if access_error in {"expired", "rotated", "missing"}:
            return render_template(
                "token_expirado.html",
                nombre=nombre,
                max_age_minutes=app.config["CLIENT_ACCESS_TOKEN_MAX_AGE"] // 60
            ), 403
        abort(403)
...
def storage(nombre):
    ...
    # Mantener la agrupacion estable por campos comunes.
    # El estado ya es independiente por fila y no debe reordenar el grupo.
    query = query.order_by(
        Pedido.fechaalta.asc(),
        Pedido.idpedido.asc(),
        func.coalesce(Pedido.proceso, '').asc(),
        func.coalesce(Pedido.nombreproveedor, '').asc(),
        Pedido.id.asc()
    )
    ...
    for p in data:
        ...
        key = f"{p.idpedido}_{p.proceso}_{p.nombreproveedor}"   # AGRUPACIÓN POR ID + PROCESO + PROVEEDOR
    ...
    marcas=marcas_json,
    can_edit_total=current_user.is_authenticated and current_user.username == "Vaello"
)
...
def update_all(nombre):
    ...
    for id_num in ids_existentes:
        ...
        numerototal = (
            int(request.form.get(f"numerototal_{id_num}", 1))
            if can_edit_total else pedido_existente.numerototal
        )

        # FOTO
        file = request.files.get(f"fotomodelo_{shared_key}")
        ...
        form_values = {
            ...
            "estadopedido": request.form.get(f"estado_{id_num}", "").strip(),
            ...
            "articulo": request.form.get(f"articulo_{shared_key}", "").strip(),
            "observaciones": request.form.get(f"observaciones_{shared_key}", "").strip(),
            "transporte": request.form.get(f"transporte_{shared_key}", "").strip(),
            "fotomodelo": ruta_foto
        }
        ...
        pedido_existente.estadopedido = form_values["estadopedido"]
        ...
        original_nombreproveedor = request.form.get(
            f"original_nombreproveedor_{id_num}",
            pedido_existente.nombreproveedor or ""
        ).strip()
        group_identity = (shared_key, original_idpedido, original_proceso, original_nombreproveedor)

        if group_identity not in grupos_actualizados:
            pedidos_mismo_grupo = pedidos_relacionados_map.get(
                (original_idpedido, original_proceso, original_nombreproveedor)
            )
            if pedidos_mismo_grupo is None:
                pedidos_mismo_grupo = (
                    ...
                    .filter(
                        ...
                        func.coalesce(Pedido.nombreproveedor, '') == original_nombreproveedor
                    )
                    .all()
                )
                pedidos_relacionados_map[
            ...
            for p in pedidos_mismo_grupo:
                ...
                p.articulo = form_values["articulo"]
                p.observaciones = form_values["observaciones"]
                p.transporte = form_values["transporte"]
                p.fotomodelo = form_values["fotomodelo"]
...
def update_pedido(id):
    ...
    original_nombreproveedor = request.form.get(
        "original_nombreproveedor",
        pedido.nombreproveedor or ""
    ).strip()
    os.makedirs(app.config['UPLOAD_FOLDER'], exist_ok=True)
    ...
    if pedido_group_id:
        ...
        for p in pedidos_relacionados:
            ...
            p.fotomodelo = pedido.fotomodelo
...
if __name__ == '__main__':
    app.config.from_object(ConfigOnline)  # Primero cargar config
    csrf.init_app(app)
    app.run()
